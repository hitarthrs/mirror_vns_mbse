    ...
    for solution in model.elements(syside.PartUsage, include_subtypes=True):
        ...
        for child in solution.owned_elements:
            ...
            try:
                ...
                continue
            # Tuple solution values (e.g. objectiveFunctionSolution[3] = (a,b,c))
            # evaluate to a list — skip those; we only want scalar features.
            if isinstance(value, (int, float)):
                values[getattr(child, "name", "?")] = float(value)
            elif isinstance(value, list):
                pass  # tuple reference — individual values read from child features
    return values
